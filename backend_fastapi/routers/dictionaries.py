...
async def dictionaries_countries(session: AsyncSession = Depends(get_session)) -> list[CountryDomain]:
    return await country_list(session)


@router.get("/dictionaries/genres/", summary='Справочник жанров')
async def dictionaries_genres(session: AsyncSession = Depends(get_session)) -> list[GenreDomain]:
    return await genres_list(session)


@router.get("/dictionaries/professions/", summary='Справочник профессий')
async def dictionaries_professions(session: AsyncSession = Depends(get_session)) -> list[ProfessionDomain]:
